...
int main(int argc, char *argv[])
{
    ...
    {
        perror("Erreur d'initialisation de SDL_Mixer : ");
        exit(EXIT_FAILURE);
    }

    Mix_VolumeMusic(MIX_MAX_VOLUME / 2);

    intro = Mix_LoadMUS("Music/Strapping_Young_Lad_You_Suck.wav");
...
void setWindow(Mix_Music *musique)
{
...
/*********************** Definition de la fenêtre *****************************************************************************/
    ...
    g_signal_connect(G_OBJECT(mainWindow), "destroy", G_CALLBACK(quit), intro);
...
/************************* Chargement de l'image a partir d'un fichier ***********************************************************/
    ...
    image = gtk_image_new_from_file("images/hack_800_405.jpg");
...
/******************************* Création des entrées de saisis ******************************************************************/
    ...
    gtk_fixed_put(GTK_FIXED(zone), srcIPEntry, 30, 90);
    gtk_widget_set_size_request(srcIPEntry, 161, 30);

    srcPortEntry = gtk_entry_new_with_max_length(5);
    gtk_fixed_put(GTK_FIXED(zone), srcPortEntry, 630, 90);
...
/********************************** Création des boutons **************************************************************************/
    ...
    gtk_fixed_put(GTK_FIXED(zone), UDPButton, 613, 275);
    ...
    gtk_fixed_put(GTK_FIXED(zone), addSrcIP, 30, 10);
    gtk_widget_set_size_request(addSrcIP, 161, 35);
    ...
    gtk_fixed_put(GTK_FIXED(zone), addSrcPort, 600, 10);
    gtk_widget_set_size_request(addSrcPort, 161, 35);
    ...
    gtk_fixed_put(GTK_FIXED(zone), addDstPort, 613, 220);
...
/******************************************* Création des labels *******************************************************************/
    ...
    gtk_fixed_put(GTK_FIXED(zone), srcIPLabel, 55, 60);
    ...
    gtk_fixed_put(GTK_FIXED(zone), srcPortLabel, 637, 60);
...
void createUDPThread(void)
{
    ...
    if(pthread_create(&udpThread, NULL, (void*)udpFlood, NULL) == -1)
    {
        threadErrorDiag();
...
void createTCPThread(void)
{
    ...
    if(pthread_create(&tcpThread, NULL, (void*)tcpFlood, NULL) == -1)
    {
        threadErrorDiag();
        //printf("Erreur création du thread udp.\nErreur Numero %d\nErreur Message %s", errno, strerror(errno));
        //exit(errno);
